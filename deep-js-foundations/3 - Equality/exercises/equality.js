...
function findAll(match, array) {
  ...
  array.forEach((type) => {
    // exact matches (`Object.is(..)`) and objects only match the exact same object
    if (Object.is(match, type)) {
      output.push(type);
    } else if (typeof match == 'string' && match.trim() != '' && type == 'number') {
      output.push(type);
    } 
    // `null` can match `undefined`, and vice versa
    else if (match == null && type == null) {
      output.push(type);
    } 
    // booleans can only match booleans
    else if (typeof match == 'boolean' && typeof type == 'boolean') {  
      if (match == type) {
        output.push(type);
      }
    }
